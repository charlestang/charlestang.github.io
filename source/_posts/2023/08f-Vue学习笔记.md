...
一款 App  里，会有一些全局设置，以及各个模块都通用的一些参数。

比如，对于大多数的功能来说，用户的身份就是一个公用的数据。可以放到一个全局的 `user` 对象中。

又比如，我本来想做一个博客编辑器，那么存储博客文章的的根目录，也是一个重要的各个模块都会用到的配置信息。

状态管理使用 `pinia`，我不明白这个为什么被称为是状态管理，从入门文档看，可以理解成一个可以 reactive 的全局变量，似乎更简单一些。

系统启动后，注入到 App 中，通过 defineStore 就可以定义一些全局对象。在使用的时候，通过 `use` 方法，再使用 `storeToRefs` 方法，就可以实现双向绑定地数据访问了。
