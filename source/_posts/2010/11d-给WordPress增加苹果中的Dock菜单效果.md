...
jQuery 有个插件，就是专门实现这个[Dock菜单的效果](http://interface.eyecon.ro/docs/fisheye)的。在[这里](http://interface.eyecon.ro/demos/fisheye.html)可以看到例子。文章标题里说的，是一个 WordPress 的插件，插件的名字翻译成中文的话，叫[动态WP鱼眼菜单](http://wordpress.org/extend/plugins/dynamicwp-fisheye-menu/)。我不知道为什么作者要取这么难听的名字，而且并不让人一目了然，既然 Dock 已经是几乎人尽皆知的效果了，何必再去自己发明名字。不过呢，这并不妨碍作者实现了这么一个使用简便的插件。到[这里](http://www.dynamicwp.net/fisheye-menu-demo/)可以看效果。
